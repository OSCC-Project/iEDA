...
void LmFeatureTiming::buildWireTimingPowerFeature(LmNet* lm_net,
  ...
  auto get_node_feature =
      [this, eval_tp, toggle, voltage, &all_node_slews](
          const auto& net_name,
          auto& lm_node) -> std::tuple<double, double, double, double, double> {
  ...
  for (auto& wire : lm_net->get_wires()) {
    ...
    auto [src_R, src_C, src_slew, src_delay, src_power] =
        get_node_feature(net_name, src);
    auto [snk_R, snk_C, snk_slew, snk_delay, snk_power] =
        get_node_feature(net_name, snk);
