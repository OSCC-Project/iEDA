...
bool LmLayoutFileIO::saveJsonNets()
{
  ...
  std::vector<std::pair<int, LmNet*>> net_vec;
  net_vec.reserve(net_map.size());
  for (auto& [net_id, lm_net] : net_map) {
    net_vec.emplace_back(net_id, &lm_net);
  }
...
#pragma omp for schedule(dynamic, 100) reduction(+ : total)
    for (int i = 0; i < (int) net_vec.size(); ++i) {
      ...
      const auto& [net_id, lm_net_ptr] = net_vec[i];
      auto& lm_net = *lm_net_ptr;
      ...
      json json_net;
      {
        ...
        /// pins
        {
          ...
          auto& pin_list = lm_net.get_pin_list();

          for (auto& [pin_id, lm_pin] : pin_list) {
            json json_pin;
            json_pin["id"] = pin_id;
...
bool LmLayoutFileIO::saveJsonPatchs()
{
  ...
  std::vector<std::pair<int, LmPatch*>> patch_vec;
...
#pragma omp for schedule(dynamic, 100)
    for (int i = 0; i < (int) patch_vec.size(); ++i) {
      ...
      json json_patch;
      {
        ...
        std::vector<std::pair<int, LmPatchLayer*>> layer_vec;
        ...
        for (const auto& [layer_id, patch_layer_ptr] : layer_vec) {
          ...
          std::vector<std::pair<int, LmNet*>> subnet_vec;
          subnet_vec.reserve(sub_nets.size());
          for (auto& [net_id, lm_net] : sub_nets) {
            subnet_vec.emplace_back(net_id, &lm_net);
          }

          for (const auto& [net_id, lm_net_ptr] : subnet_vec) {
            auto& lm_net = *lm_net_ptr;
            json json_net = {};
            json_net["id"] = net_id;
