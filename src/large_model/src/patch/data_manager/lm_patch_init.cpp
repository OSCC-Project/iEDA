...
{
  auto init_patch = [&](int patch_row, int patch_col) -> LmPatch {
    LmPatch im_patch;
  ...
  LOG_INFO << "LM patch grid init end...";
}

void LmPatchInit::initSubNet()
{
  ieda::Stats stats;

  LOG_INFO << "LM patch init subnet start...";
...
#pragma omp parallel for schedule(dynamic)
  ...
  LOG_INFO << "LM memory usage " << stats.memoryDelta() << " MB";
  LOG_INFO << "LM elapsed time " << stats.elapsedRunTime() << " s";
  LOG_INFO << "LM patch init subnet end...";
}
