...
void LmNetWire::addPatch(int patch_id, int layer_id)
{
...
LmNetFeature* LmNet::get_feature(bool b_create)
{
  if (_feature == nullptr && b_create) {
    _feature = new LmNetFeature;
  }
  return _feature;
}

LmNetWire* LmNet::findWire(int64_t wire_id)
{
  auto result = std::find_if(_wires.begin(), _wires.end(), [wire_id](LmNetWire wire) { return wire.get_id() == wire_id; });
