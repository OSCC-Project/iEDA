...
namespace idb {
  ...
  int32_t LayoutWrite::save_rows() {
    ...
    shared_ptr<IdbRowsHeader> header =
        make_shared<IdbRowsHeader>(IdbFileHeaderType::kRows, file_path.c_str(), rows, nullptr);
  ...
  int32_t LayoutWrite::save_via_list() {
    ...
    shared_ptr<IdbViaListHeader> header =
        make_shared<IdbViaListHeader>(IdbFileHeaderType::kVias, file_path.c_str(), via_list, nullptr);
