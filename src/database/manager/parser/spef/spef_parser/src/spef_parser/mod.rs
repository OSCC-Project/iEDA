...
use spef_data::{
    SpefConnEntry, SpefEntryBasicInfo, SpefExchange, SpefFloatValue, SpefHeaderEntry, SpefNameMapEntry, SpefNet,
    SpefParserData, SpefPortEntry, SpefStringValue,
};
use std::fs;
...
mod ffi {
    ...
    extern "Rust" {
        fn parse_spef_file(path: &str) -> bool;
    }
}
...
pub fn parse_spef_file(spef_file_path: &str) -> Result<spef_data::SpefParserData, pest::error::Error<Rule>> {
    ...
    let mut spef_file: ffi::SpefFile;
    ...
    for entry in spef_entries {
        entry_parse_result = match entry.as_rule() {
            Rule::section => {
    ...
    // println!("{entry_parse_result:#?}");
    match entry_parse_result {
        Ok(value) => Ok(value),
