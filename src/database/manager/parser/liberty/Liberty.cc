 ...
 */
double LibertyArc::getDelayOrConstrainCheckNs(TransType trans_type, double slew,
  ...
  if (isDelayArc()) {
    return _table_model->gateDelay(trans_type, slew * input_to_liberty_convert,
                                   load_or_constrain_slew) *
           liberty_to_output_convert;
  } else {
    return _table_model->gateCheckConstrain(
