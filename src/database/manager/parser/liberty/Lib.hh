...
class LibObject {
 public:
  ...
  virtual void addAxis(std::unique_ptr<LibAxis>&& axis) {
...
class LibAxis : public LibObject {
 public:
  ...
  void set_axis_values(
      std::vector<std::unique_ptr<LibAttrValue>>&& table_values) {
 ...
 private:
  ...
  std::vector<std::unique_ptr<LibAttrValue>>
...
class LibTable : public LibObject {
 public:
  ...
  void addAxis(std::unique_ptr<LibAxis>&& axis) override {
...
class LibCurrentData {
 public:
  LibCurrentData(LibVectorTable* low_low, LibVectorTable* low_high,
                 LibVectorTable* high_low, LibVectorTable* high_high,
...
class LibCCSTable : public LibObject {
 public:
  ...
  void addTable(std::unique_ptr<LibVectorTable>&& current_table) {
 ...
 private:
  ...
  std::vector<std::unique_ptr<LibVectorTable>>
...
class LibTableModel : public LibObject {
 public:
  ...
  virtual std::optional<double> gateDelay(TransType trans_type, double slew,
                                          double load) {
    LOG_FATAL << "not support";
    return 0.0;
  }
  ...
  virtual std::unique_ptr<LibCurrentData> gateOutputCurrent(
...
class LibDelayTableModel final : public LibTableModel {
 ...
 private:
  std::array<std::unique_ptr<LibTable>, kTableNum>
...
class LibCheckTableModel final : public LibTableModel {
 public:
  ...
  LibTable* getTable(int index) override { return _tables[index].get(); }
  std::optional<double> gateCheckConstrain(TransType trans_type, double slew,
...
class LibInternalPowerInfo {
 public:
  ...
  void set_power_table_model(
      std::unique_ptr<LibTableModel>&& power_table_model) {
...
class LibPort : public LibObject {
 public:
  ...
  void addInternalPower(
      std::unique_ptr<LibInternalPowerInfo>&& internal_power) {
 ...
 private:
  ...
  Vector<std::unique_ptr<LibInternalPowerInfo>>
...
class LibType : public LibObject {
 public:
  explicit LibType(std::string&& type_name)
...
class LibPortBus : public LibPort {
 public:
  ...
  void addlibertyPort(std::unique_ptr<LibPort>&& port) {
...
class LibArc : public LibObject {
 public:
  ...
  void set_table_model(std::unique_ptr<LibTableModel>&& table_model) {
...
class LibPowerArc : public LibObject {
 public:
  ...
  LibTableModel* get_power_table_model() {
    return _internal_power_info->get_power_table_model();
  }

  void set_internal_power_info(
      std::unique_ptr<LibInternalPowerInfo>&& internal_power_info) {
 ...
 private:
  ...
  std::unique_ptr<LibInternalPowerInfo>
...
class LibCell : public LibObject {
 public:
  ...
  void addLeakagePower(std::unique_ptr<LibLeakagePower>&& leakage_power) {
  ...
  std::optional<LibArcSet*> findLibertyArcSet(const char* from_port_name,
  ...
  std::optional<LibPowerArcSet*> findLibertyPowerArcSet(
 ...
 private:
  ...
  std::vector<std::unique_ptr<LibLeakagePower>>
  ...
  std::vector<std::unique_ptr<LibArcSet>>
      _cell_arcs;  //!< All timing arcs of the cell.
  std::vector<std::unique_ptr<LibPowerArcSet>>
...
#define FOREACH_CELL_PORT(cell, port)                                   \
  for (std::vector<std::unique_ptr<ista::LibPort>>::iterator iter =     \
           cell->get_cell_ports().begin();                              \
       (iter != cell->get_cell_ports().end()) ? port = (iter++->get()), \
                                                             true : false;)
...
#define FOREACH_CELL_TIMING_ARC_SET(cell, timing_arc_set)                    \
  for (std::vector<std::unique_ptr<LibArcSet>>::iterator iter =              \
           cell->get_cell_arcs().begin();                                    \
       iter != cell->get_cell_arcs().end() ? timing_arc_set = iter++->get(), \
                                                         true : false;)
...
#define FOREACH_POWER_ARC_SET(cell, power_arc_set)                         \
  for (std::vector<std::unique_ptr<ista::LibPowerArcSet>>::iterator iter = \
           cell->get_cell_power_arcs().begin();                            \
       iter != cell->get_cell_power_arcs().end()                           \
       ? power_arc_set = iter++->get(),                                    \
                                                                    true   \
       : false;)
...
class LibCurrentTemplate : public LibLutTableTemplate {
 public:
  ...
  void set_template_axis(std::unique_ptr<LibAxis>&& axis) {
...
class LibLibrary : public LibObject {
 ...
 private:
  ...
  Vector<std::unique_ptr<LibLutTableTemplate>>
...
#define FOREACH_LIB_CELL(lib, cell)                           \
  for (std::vector<std::unique_ptr<LibCell>>::iterator iter = \
           lib->get_cells().begin();                          \
...
class LibBuilder {
 public:
  ...
  explicit LibBuilder(const char* lib_name)
      : _lib(std::make_unique<LibLibrary>(lib_name)) {}
  ~LibBuilder() = default;
  ...
  void set_leakage_power(LibLeakagePower* leakage_power) {
  ...
  void set_table_model(LibTableModel* table_model) {
