...
LibTable::LibTable(TableType table_type, LibLutTableTemplate* table_template)
 ...
 * @return LibAxis&
 */
LibAxis& LibTable::getAxis(unsigned int index) { return *(get_axes()[index]); }
...
double LibTable::findValue(double slew, double constrain_slew_or_load) {
  ...
  switch (*(table_template->get_template_variable1())) {
    ...
    case LibLutTableTemplate::Variable::INPUT_TRANSITION_TIME:
      if (auto variable2 = table_template->get_template_variable2();
        ...
        LOG_FATAL_IF(
            ...
            *variable2 !=
                LibLutTableTemplate::Variable::CONSTRAINED_PIN_TRANSITION);
      }
    ...
    case LibLutTableTemplate::Variable::CONSTRAINED_PIN_TRANSITION:
      if (auto variable2 = table_template->get_template_variable2();
          variable2) {
        LOG_FATAL_IF(
            *variable2 != LibLutTableTemplate::Variable::INPUT_NET_TRANSITION &&
            *variable2 !=
  ...
  if (1 == get_axes().size()) {
    ...
    auto result =
        BilinearInterpolation(q11, q12, q21, q22, x1, x2, y1, y2, val1, val2);
...
double LibTable::driveResistance() {
  ...
  if (table_template) {
    ...
    if (var1 &&
        *var1 == LibLutTableTemplate::Variable::TOTAL_OUTPUT_NET_CAPACITANCE) {
      auto num_val1 = getAxis(0).get_axis_size();
      out_cap = getAxis(0)[num_val1 - 1];
      out_slew = findValue(0.0, out_cap);
    } else if (var2 && *var2 == LibLutTableTemplate::Variable::
...
LibVectorTable::LibVectorTable(TableType table_type,
                               LibLutTableTemplate* table_template)
    : LibTable(table_type, table_template) {}

LibVectorTable::LibVectorTable(LibVectorTable&& other) noexcept
    : LibTable(std::move(other)), _ref_time(other._ref_time) {}
 ...
 */
std::vector<double> LibVectorTable::getOutputCurrent(
  ...
  auto get_time_index = [&time_axis_value](double current_time,
    ...
    while (start_index < axis_size) {
      ...
      if ((time_value > current_time) ||
          IsDoubleEqual(time_value, current_time, 0.000000001)) {
        break;
      }
      ++start_index;
    }
  ...
  double interval = (simu_info->_end_time - simu_info->_start_time) /
                    (simu_info->_num_sim_point - 1);

  int start_index = 0;
  for (double current_time = start_time;
       (current_time < end_time) ||
       IsDoubleEqual(current_time, end_time, 0.000000001);
    ...
    double output_current = LinearInterpolate(
        lower_time, upper_time, lower_current, upper_current, current_time);
    output_currents.push_back(output_current);
  }

  LOG_FATAL_IF(simu_info->_num_sim_point != output_currents.size())
      << "output currents size is not equal sim point num.";

  return output_currents;
}

LibCurrentData::LibCurrentData(LibVectorTable* low_low,
                               LibVectorTable* low_high,
                               LibVectorTable* high_low,
                               LibVectorTable* high_high, double slew,
                               double load)
    : _low_low(low_low),
      _low_high(low_high),
      _high_low(high_low),
 ...
 *
 */
std::vector<double> LibCurrentData::getOutputCurrent(
    std::optional<LibCurrentSimuInfo>& simu_info) {
  ...
  for (size_t index = 0; index < min_size; ++index) {
    if (IsDoubleEqual(low_slew, high_slew)) {
      output_currents.push_back(
          LinearInterpolate(low_load, high_load, low_low_current[index],
                            low_high_current[index], _load));
    } else if (IsDoubleEqual(low_load, high_load)) {
      output_currents.push_back(
          LinearInterpolate(low_slew, high_slew, low_low_current[index],
                            high_low_current[index], _slew));
    } else {
      auto output_current = BilinearInterpolation(
...
LibCCSTable::LibCCSTable(LibTable::TableType table_type)
    : _table_type(table_type) {}

LibDelayTableModel::LibDelayTableModel(LibDelayTableModel&& other) noexcept
    : _tables(std::move(other._tables)) {}

LibDelayTableModel& LibDelayTableModel::operator=(
    LibDelayTableModel&& rhs) noexcept {
  if (this != &rhs) {
 ...
 */
std::optional<double> LibDelayTableModel::gateDelay(TransType trans_type,
 ...
 */
std::optional<double> LibDelayTableModel::gateSlew(TransType trans_type,
                                                   double slew, double load) {
  LibTable* table = nullptr;
  if (trans_type == TransType::kRise) {
 ...
 * @return double
 */
std::unique_ptr<LibCurrentData> LibDelayTableModel::gateOutputCurrent(
  ...
  auto current_data = std::make_unique<LibCurrentData>(
...
double LibDelayTableModel::driveResistance() {
  ...
  LibTable* rise_table =
      getTable(CAST_TYPE_TO_INDEX(LibTable::TableType::kRiseTransition));
  if (rise_table) {
    rise_resistance = rise_table->driveResistance();
  }

  LibTable* fall_table =
      getTable(CAST_TYPE_TO_INDEX(LibTable::TableType::kFallTransition));
  if (fall_table) {
 ...
 */
std::optional<double> LibCheckTableModel::gateCheckConstrain(
    TransType trans_type, double slew, double constrain_slew) {
  LibTable* table = nullptr;
  if (trans_type == TransType::kRise) {
...
LibCheckTableModel::LibCheckTableModel(LibCheckTableModel&& other) noexcept
    : _tables(std::move(other._tables)) {}

LibCheckTableModel& LibCheckTableModel::operator=(
    LibCheckTableModel&& rhs) noexcept {
  if (this != &rhs) {
    _tables = std::move(rhs._tables);
  }
...
LibPowerTableModel::LibPowerTableModel(LibPowerTableModel&& other) noexcept
    : _tables(std::move(other._tables)) {}

LibPowerTableModel& LibPowerTableModel::operator=(
    LibPowerTableModel&& rhs) noexcept {
  if (this != &rhs) {
    _tables = std::move(rhs._tables);
  }
 ...
 */
double LibPowerTableModel::gatePower(TransType trans_type, double slew,
 ...
 * @param cap
 */
void LibPort::set_port_cap(AnalysisMode mode, TransType trans_type,
                           double cap) {
  if (IS_MAX(mode)) {
 ...
 * @return double
 */
std::optional<double> LibPort::get_port_cap(AnalysisMode mode,
...
BTreeMap<std::string, LibArc::TimingType> LibArc::_str_to_type = {
 ...
 */
double LibArc::getDelayOrConstrainCheckNs(TransType trans_type, double slew,
                                          double load_or_constrain_slew) {
  // get/set time unit of liberty and derate
  TimeUnit input_time_unit = TimeUnit::kNS;
  TimeUnit liberty_time_unit =
      get_owner_cell()->get_owner_lib()->get_time_unit();
  ...
  if (isDelayArc()) {
    found_delay = _table_model->gateDelay(
        trans_type, slew * input_to_liberty_convert, load_or_constrain_slew);
  } else {
    found_delay = _table_model->gateCheckConstrain(
        trans_type, slew * input_to_liberty_convert, load_or_constrain_slew);
  }
...
double LibArc::getSlewNs(TransType trans_type, double slew, double load) {
  ...
  auto found_slew =
      _table_model->gateSlew(trans_type, slew * input_to_liberty_convert, load);

  if (found_slew) {
    double ret_value =
        (*found_slew) * liberty_to_output_convert * slew_derate_from_library;
    return ret_value;
  }
 ...
 */
std::unique_ptr<LibCurrentData> LibArc::getOutputCurrent(TransType trans_type,
                                                         double slew,
                                                         double load) {
  if (!isDelayArc()) {
...
LibArcSet::LibArcSet(LibArcSet&& other) noexcept
...
LibPowerArcSet::LibPowerArcSet(LibPowerArcSet&& other) noexcept
...
void LibCell::addLibertyPowerArc(
    std::unique_ptr<LibPowerArc>&& cell_power_arc) {
 ...
 */
std::optional<LibArcSet*> LibCell::findLibertyArcSet(
 ...
 * @return unsigned 1 if success, 0 else fail.
 */
std::optional<LibArcSet*> LibCell::findLibertyArcSet(const char* from_port_name,
 ...
 * @return unsigned 1 if success, 0 else fail.
 */
std::optional<LibPowerArcSet*> LibCell::findLibertyPowerArcSet(
    const char* from_port_name, const char* to_port_name) {
...
LibWireLoad::LibWireLoad(const char* wire_load_name)
    : _wire_load_name(wire_load_name) {}

LibLutTableTemplate::LibLutTableTemplate(const char* template_name)
    : _template_name(template_name) {}

const std::map<std::string_view, LibLutTableTemplate::Variable>
    LibLutTableTemplate::_str2var = {
        {"total_output_net_capacitance",
         LibLutTableTemplate::Variable::TOTAL_OUTPUT_NET_CAPACITANCE},
        ...
        {"input_noise_height",
         LibLutTableTemplate::Variable::INPUT_NOISE_HEIGHT},
        {"input_noise_width", LibLutTableTemplate::Variable::INPUT_NOISE_WIDTH},
        {"normalized_voltage",
         LibLutTableTemplate::Variable::NORMALIZED_VOLTAGE}};

LibCurrentTemplate::LibCurrentTemplate(const char* template_name)
    : LibLutTableTemplate(template_name) {}
