...
fn process_port_connect(
    ...
    let mut net_expr_id = net_expr.get_verilog_id();
    ...
    if !verilog_data::VerilogModule::is_port(&cur_module.borrow(), net_base_name) {
        ...
        if range.is_none() {
            ...
            while is_first_greater && index >= range.unwrap().1 || !is_first_greater && index <= range.unwrap().1 {
                ...
                let new_index_net_id = Box::new(verilog_data::VerilogNetIDExpr::new(0, dyn_index_id));
                let verilog_virtual_base_net_expr: Box<dyn verilog_data::VerilogVirtualBaseNetExpr> =
            ...
            let new_concat_net_id = verilog_data::VerilogNetConcatExpr::new(0, verilog_id_concat);
    ...
    match parse_result {
        Ok(pairs) => {
            ...
            for pair in pairs {
                ...
                let verilog_module =
                    Rc::new(RefCell::new(verilog_data::VerilogModule::new(1, module_name, port_list, module_stmts)));
