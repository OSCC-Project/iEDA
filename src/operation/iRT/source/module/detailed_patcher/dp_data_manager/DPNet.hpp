...
class DPNet
{
 public:
  ...
  std::vector<DPPin>& get_dp_pin_list() { return _dp_pin_list; }
  // setter
  void set_origin_net(Net* origin_net) { _origin_net = origin_net; }
  void set_net_idx(const int32_t net_idx) { _net_idx = net_idx; }
  void set_connect_type(const ConnectType& connect_type) { _connect_type = connect_type; }
  void set_dp_pin_list(const std::vector<DPPin>& dp_pin_list) { _dp_pin_list = dp_pin_list; }

 private:
  Net* _origin_net = nullptr;
  int32_t _net_idx = -1;
  ConnectType _connect_type = ConnectType::kNone;
  std::vector<DPPin> _dp_pin_list;
};
