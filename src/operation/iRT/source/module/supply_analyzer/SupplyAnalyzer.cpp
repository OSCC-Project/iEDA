...
std::vector<LayerRect> SupplyAnalyzer::getCrossingWireList(irt_int layer_idx, SANode& first_node, SANode& second_node)
{
  std::vector<RoutingLayer>& routing_layer_list = DM_INST.getDatabase().get_routing_layer_list();
  RoutingLayer& routing_layer = routing_layer_list[layer_idx];
  ...
  if (routing_layer.isPreferH()) {
    for (irt_int y : RTUtil::getOpenScaleList(real_lb_y, real_rt_y, routing_layer.getYTrackGridList())) {
      wire_list.emplace_back(real_lb_x, y - half_width, real_rt_x, y + half_width, layer_idx);
    }
  } else {
    for (irt_int x : RTUtil::getOpenScaleList(real_lb_x, real_rt_x, routing_layer.getXTrackGridList())) {
  ...
  for (SANode* node : {&first_node, &second_node}) {
    for (auto& [net_idx, fixed_rect_set] : node->get_net_fixed_rect_map()) {
      for (auto& fixed_rect : fixed_rect_set) {
        irt_int enlarged_size = routing_layer.getMinSpacing(fixed_rect->get_real_rect());
  ...
  for (size_t layer_idx = 0; layer_idx < layer_node_map.size(); layer_idx++) {
    ...
    for (irt_int grid_x = 0; grid_x < sa_node_map.get_x_size(); grid_x++) {
      for (irt_int grid_y = 0; grid_y < sa_node_map.get_y_size(); grid_y++) {
        ...
        irt_int y_reduced_span = shape.getYSpan() / 25;
        irt_int y = shape.get_rt_y();
        ...
        if (!sa_node.get_orien_supply_map().empty()) {
          ...
          gp_text_orien_supply_map_info.set_text_type(static_cast<irt_int>(GPDataType::kInfo));
          ...
          gp_text_orien_supply_map_info.set_layer_idx(GP_INST.getGDSIdxByRouting(static_cast<irt_int>(layer_idx)));
...
void SupplyAnalyzer::writeSupplyCSV(SAModel& sa_model)
{
  ...
  for (RoutingLayer& routing_layer : routing_layer_list) {
    ...
    for (irt_int y = gcell_map.get_y_size() - 1; y >= 0; y--) {
      for (irt_int x = 0; x < gcell_map.get_x_size(); x++) {
