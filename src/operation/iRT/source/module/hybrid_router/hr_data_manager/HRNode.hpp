...
class HRNode : public LayerCoord
{
 public:
  HRNode() = default;
  ~HRNode() = default;
  // getter
  std::map<Orientation, HRNode*>& get_neighbor_node_map() { return _neighbor_node_map; }
  ...
  // setter
  void set_neighbor_node_map(const std::map<Orientation, HRNode*>& neighbor_node_map) { _neighbor_node_map = neighbor_node_map; }
  ...
  // function
  HRNode* getNeighborNode(Orientation orientation)
  {
    HRNode* neighbor_node = nullptr;
  ...
  bool isOpen() { return _state == HRNodeState::kOpen; }
  bool isClose() { return _state == HRNodeState::kClose; }
  double getTotalCost() { return (_known_cost + _estimated_cost); }
#endif

 private:
  std::map<Orientation, HRNode*> _neighbor_node_map;
  // obstacle & pin_shape
...
#if 1  // astar
struct CmpHRNodeCost
{
  bool operator()(HRNode* a, HRNode* b)
  {
