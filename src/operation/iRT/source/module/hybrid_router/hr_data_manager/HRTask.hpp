...
class HRTask
{
 public:
  ...
  std::vector<HRGroup>& get_hr_group_list() { return _hr_group_list; }
  ...
  const std::vector<HRGroup>& get_hr_group_list() const { return _hr_group_list; }
  const PlanarRect& get_bounding_box() const { return _bounding_box; }
  // setter
  void set_net_idx(const int32_t net_idx) { _net_idx = net_idx; }
  void set_connect_type(const ConnectType& connect_type) { _connect_type = connect_type; }
  void set_hr_group_list(const std::vector<HRGroup>& hr_group_list) { _hr_group_list = hr_group_list; }
 ...
 private:
  ...
  std::vector<HRGroup> _hr_group_list;
...
struct CmpHRTask
{
  bool operator()(const HRTask* a, const HRTask* b) const
  {
    ...
    // PinNum 降序
    if (sort_status == SortStatus::kEqual) {
      int32_t a_pin_num = static_cast<int32_t>(a->get_hr_group_list().size());
      int32_t b_pin_num = static_cast<int32_t>(b->get_hr_group_list().size());
