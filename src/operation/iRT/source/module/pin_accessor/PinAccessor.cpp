...
std::map<PANode*, std::set<Orientation>> PinAccessor::getRoutingNodeOrientationMap(PABox& pa_box, NetShape& net_shape)
{
  ...
  for (auto& [x_spacing, y_spacing] : spacing_pair_list) {
    ...
    RTUTIL.getTrackIndexSet(pa_box.get_box_track_axis().get_x_grid_list(), planar_enlarged_rect.get_ll_x(), planar_enlarged_rect.get_ur_x(), x_pre_set, x_mid_set, x_post_set);

    std::set<int32_t> y_pre_set;
    std::set<int32_t> y_mid_set;
    std::set<int32_t> y_post_set;
    RTUTIL.getTrackIndexSet(pa_box.get_box_track_axis().get_y_grid_list(), planar_enlarged_rect.get_ll_y(), planar_enlarged_rect.get_ur_y(), y_pre_set, y_mid_set, y_post_set);
  ...
  for (auto& [x_spacing, y_spacing] : spacing_pair_list) {
    ...
    std::set<int32_t> x_mid_set;
    std::set<int32_t> x_post_set;
    RTUTIL.getTrackIndexSet(pa_box.get_box_track_axis().get_x_grid_list(), space_enlarged_rect.get_ll_x(), space_enlarged_rect.get_ur_x(), x_pre_set, x_mid_set, x_post_set);

    std::set<int32_t> y_pre_set;
    std::set<int32_t> y_mid_set;
    std::set<int32_t> y_post_set;
    RTUTIL.getTrackIndexSet(pa_box.get_box_track_axis().get_y_grid_list(), space_enlarged_rect.get_ll_y(), space_enlarged_rect.get_ur_y(), y_pre_set, y_mid_set, y_post_set);
