...
class VRTask
{
 public:
  ...
  std::vector<VRGroup>& get_vr_group_list() { return _vr_group_list; }
  ...
  const std::vector<VRGroup>& get_vr_group_list() const { return _vr_group_list; }
  const PlanarRect& get_bounding_box() const { return _bounding_box; }
  // setter
  void set_net_idx(const int32_t net_idx) { _net_idx = net_idx; }
  void set_connect_type(const ConnectType& connect_type) { _connect_type = connect_type; }
  void set_vr_group_list(const std::vector<VRGroup>& vr_group_list) { _vr_group_list = vr_group_list; }
 ...
 private:
  ...
  std::vector<VRGroup> _vr_group_list;
...
struct CmpVRTask
{
  bool operator()(const VRTask* a, const VRTask* b) const
  {
    ...
    // PinNum 降序
    if (sort_status == SortStatus::kEqual) {
      int32_t a_pin_num = static_cast<int32_t>(a->get_vr_group_list().size());
      int32_t b_pin_num = static_cast<int32_t>(b->get_vr_group_list().size());
