...
std::vector<double> ir_cg_solver(Eigen::SparseMatrix<double> &A,
  ...
  cudaMalloc((void **)&d_csrRowPtr, (num_rows + 1) * sizeof(int));
  cudaMalloc((void **)&d_csrColInd, nnz * sizeof(int));
  cudaMalloc((void **)&d_b, num_rows * sizeof(double));
  cudaMalloc((void **)&d_x, num_rows * sizeof(double));
  cudaMalloc((void **)&d_r, num_rows * sizeof(double));
  cudaMalloc((void **)&d_p, num_rows * sizeof(double));
  cudaMalloc((void **)&d_Ap, num_rows * sizeof(double));

  // Copy data to device
  cudaMemcpy(d_csrVal, csrVal, nnz * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(d_csrRowPtr, csrRowPtr, (num_rows + 1) * sizeof(int),
             cudaMemcpyHostToDevice);
  cudaMemcpy(d_csrColInd, csrColInd, nnz * sizeof(int), cudaMemcpyHostToDevice);
  cudaMemcpy(d_b, b.data(), num_rows * sizeof(double), cudaMemcpyHostToDevice);
  cudaMemcpy(d_x, x0.data(), num_rows * sizeof(double), cudaMemcpyHostToDevice);
  ...
  cusparseDnVecDescr_t vecX, vecR, vecAp;
  ...
  cudaMemcpy(d_p, d_r, num_rows * sizeof(double), cudaMemcpyDeviceToDevice);
  ...
  while (k < max_iter && sqrt(r_dot_r) > tol) {
    // Ap = A * p
    cusparseSpMV(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, &alpha, matA, vecAp,
                 &beta, vecAp, CUDA_R_64F, CUSPARSE_SPMV_ALG_DEFAULT, dBuffer);
    ...
    cublasDaxpy(cublasHandle, num_rows, &alpha, d_r, 1, d_p, 1);
