...
class CtsCellLib {
 public:
  CtsCellLib(const std::string& cell_master,
             const std::vector<std::vector<double>>& index_list,
             const std::vector<double>& delay_values,
             const std::vector<double>& slew_values)
      : _cell_master(cell_master),
        _index_list(index_list),
        _delay_values(delay_values),
        _slew_values(slew_values) {
    _slew_in_span =
        std::span<const double>(_index_list[0].data(), _index_list[0].size());
    _cap_out_span =
        std::span<const double>(_index_list[1].data(), _index_list[1].size());
  }
  ~CtsCellLib() = default;
  // get
  std::string get_cell_master() const { return _cell_master; }
  std::vector<std::vector<double>> get_index_list() const {
    return _index_list;
  }
...
#ifdef PY_MODEL
  void set_delay_lib_model(ModelBase* delay_lib_model) {
    _delay_lib_model = delay_lib_model;
  }
  void set_slew_lib_model(ModelBase* slew_lib_model) {
    _slew_lib_model = slew_lib_model;
  }
#endif
  ...
  double calcLinearSlew(const double& cap_out) const {
    return _slew_coef[0] + _slew_coef[1] * cap_out;
  }
  ...
  double calcInsertDelay(const double& slew_in, const double& cap_out) const {
    // slew_in index
    auto it1 =
        std::upper_bound(_slew_in_span.begin(), _slew_in_span.end(), slew_in);
    size_t i1 = std::clamp(
        static_cast<size_t>(std::distance(_slew_in_span.begin(), it1)),
        size_t{1}, _slew_in_span.size() - 1);
    double slew_low = _slew_in_span[i1 - 1];
    double slew_high = _slew_in_span[i1];

    // cap_out index
    auto it2 =
        std::upper_bound(_cap_out_span.begin(), _cap_out_span.end(), cap_out);
    size_t i2 = std::clamp(
        static_cast<size_t>(std::distance(_cap_out_span.begin(), it2)),
        size_t{1}, _cap_out_span.size() - 1);
    ...
    double res = std::lerp(
        std::lerp(
            y11, y12,
            std::lerp(0.0, 1.0, (cap_out - cap_low) / (cap_high - cap_low))),
        std::lerp(
            y21, y22,
            std::lerp(0.0, 1.0, (cap_out - cap_low) / (cap_high - cap_low))),
...
class CtsLibs {
 public:
  CtsLibs() = default;
  ~CtsLibs() = default;
  void insertLib(const std::string& cell_master, CtsCellLib* lib) {
    _lib_maps[cell_master] = lib;
  }
  ...
  void insertModel(const std::string& net_name, ModelBase* model) {
    _model_maps[net_name] = model;
  }
