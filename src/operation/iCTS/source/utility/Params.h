...
enum class DelayModel { kLINEAR = 0, kELMORE = 1 };
class Params {
 public:
  Params() : _model(DelayModel::kLINEAR), _unit_res(0), _unit_cap(0) {}
  Params(DelayModel model, int db_unit, double unit_res = 1,
         double unit_cap = 1)
      : _model(model),
        _db_unit(db_unit),
        _unit_res(unit_res),
...
class ZstParams : public Params {
 public:
  ...
  ZstParams() : Params() {}
  ZstParams(DelayModel model, int db_unit, double unit_res = 0,
            double unit_cap = 0)
      : Params(model, db_unit, unit_res, unit_cap) {}
};

enum class BstType { kBME = 0, kIME = 1 };
class BstParams : public Params {
 public:
  typedef BstTag DmeTag;

  BstParams() : Params(), _type(BstType::kBME), _skew_bound(0) {}
  BstParams(BstType type, int db_unit, double skew_bound, DelayModel model,
            double unit_res = 0, double unit_cap = 0)
      : Params(model, db_unit, unit_res, unit_cap),
        _type(type),
...
class UstParams : public Params {
 public:
  ...
  UstParams() : Params() {}
  UstParams(DelayModel model, int db_unit, double skew_bound,
            double unit_res = 0, double unit_cap = 0)
