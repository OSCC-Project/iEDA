...
void TreeSaltBuilder::dfs(const std::shared_ptr<salt::TreeNode>& tree_node, const std::shared_ptr<salt::TreeNode>& beat_node, double eps)
{
  if (tree_node->pin && _cur_latency[beat_node->id] > (1 + eps) * _shortest_latency[beat_node->id]) {
...
void TempBuilder::dfs(const std::shared_ptr<salt::TreeNode>& tree_node, const std::shared_ptr<salt::TreeNode>& beat_node, double eps)
{
  if (tree_node->pin && _cur_latency[beat_node->id] > (1 + eps) * _shortest_latency[beat_node->id]) {
