...
#define WARNING(...) LOG_INST.warning(Loc::current(), ##__VA_ARGS__)
#define ERROR(...) LOG_INST.error(Loc::current(), ##__VA_ARGS__)
class Logger
{
 public:
  ...
  // function
  int getLogLevel() const { return _log_level; }
  void setLogLevel(const int log_level) { _log_level = log_level; }
 ...
 private:
  ...
  void printLog(LogLevel log_level, Loc location, const T& value, const Args&... args)
  {
    ...
    std::string header = getString("[MP ", getTimestamp(), " ", file_name, " ", location.function_name(), " ");
