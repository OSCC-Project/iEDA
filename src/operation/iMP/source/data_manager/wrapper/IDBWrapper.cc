 ...
 * @Description:
 * @FilePath: /iEDA/src/imp/src/wrapper/IDBWrapper.cc
...
IDBWrapper::IDBWrapper(IdbBuilder* idb_builder) : IDBWrapper()
{
  setIdbBuilder(idb_builder);
}

IDBWrapper::IDBWrapper(const std::string& idb_json) : IDBWrapper()
{
  dmInst->init(idb_json);
  setIdbBuilder(dmInst->get_idb_builder());
}
...
void IDBWrapper::setIdbBuilder(IdbBuilder* idb_builder)
{
...
void IDBWrapper::deleteInstsForTest()
{
  ...
  std::map<imp::Instance*, idb::IdbInstance*> new_map;
...
Pin* IDBWrapper::wrapPin(IdbPin* idb_pin)
{
  ...
  LOG_ERROR_IF(!pin_ptr) << "Fail on creating imp PIN!";
...
void IDBWrapper::initInstancesForFragmentedRow()
{
  ...
  auto fastModulo = [](const int input, const int ceil) { return input >= ceil ? input % ceil : input; };
  auto obtainMinMaxIdx = [&](int border_bottom, int interval, int inquiry_bottom, int inquiry_top) {
  ...
  for (auto* row : row_list) {
    ...
    std::pair<int, int> pair_x = obtainMinMaxIdx(core_lower.get_x(), site_width, row_shape.get_ll_x(), row_shape.get_ur_x());
    std::pair<int, int> pair_y = obtainMinMaxIdx(core_lower.get_y(), row_height, row_shape.get_ll_y(), row_shape.get_ur_y());
  ...
  for (auto inst : ipl_design->get_instance_list()) {
    ...
    if (inst->isFixed()) {
      ...
      std::pair<int, int> pair_x = obtainMinMaxIdx(core_lower.get_x(), site_width, inst_shape.get_ll_x(), inst_shape.get_ur_x());
      std::pair<int, int> pair_y = obtainMinMaxIdx(core_lower.get_y(), row_height, inst_shape.get_ll_y(), inst_shape.get_ur_y());
