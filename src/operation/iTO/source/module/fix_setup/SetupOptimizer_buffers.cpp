...
void SetupOptimizer::performVGBuffering(Pin* pin, int& num_insert_buf)
{
  ...
  tree->updateBranch();
  BufferedOptionSeq buf_solotions = findBufferSolution(tree, tree->left(driver_id), driver_id);
...
BufferedOptionSeq SetupOptimizer::findBufferSolution(RoutingTree* tree, int curr_id, int prev_id)
{
  ...
  if (obj_pin) {
    ...
    if (timingEngine->get_sta_engine()->isLoad(pin->getFullName().c_str())) {
      BufferedOption* buffered_option = new BufferedOption(BufferedOptionType::kLoad);
...
BufferedOptionSeq SetupOptimizer::mergeBranch(BufferedOptionSeq buf_opt_left, BufferedOptionSeq buf_opt_right, Point curr_loc)
{
...
BufferedOptionSeq SetupOptimizer::addWire(BufferedOptionSeq buf_opt_seq, Point curr_loc, Point prev_loc)
{
...
BufferedOptionSeq SetupOptimizer::addBuffer(BufferedOptionSeq buf_opt_seq, Point prev_loc)
{
