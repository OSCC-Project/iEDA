...
double TimingIDBAdapter::getCapacitance(int num_layer, double segment_length,
  ...
  double lef_capacitance;
  double lef_edge_capacitance;
  if (idb_layout->get_units()->get_picofarads() == -1) {
    lef_capacitance = routing_layer->get_capacitance();
    lef_edge_capacitance = routing_layer->get_edge_capacitance();
  } else {
    lef_capacitance = routing_layer->get_capacitance() /
                      idb_layout->get_units()->get_picofarads();
    lef_edge_capacitance = routing_layer->get_edge_capacitance() /
                           idb_layout->get_units()->get_picofarads();
  }
