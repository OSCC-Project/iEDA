...
void RuleValidator::verifyMinimumArea(RVBox& rv_box)
{
  ...
  // 遍历每个图形，如果面积小于min area则添加到violation list中
    ...
    for (auto& [net_idx, poly_set] : net_poly_set) {
      ...
      for (GTLPolyInt poly : poly_list) {
        if (gtl::area(poly) < min_area) {
          ...
          gtl::get_max_rectangles(violation_rect_list, poly);
          std::sort(violation_rect_list.begin(), violation_rect_list.end(),
                    [](const GTLRectInt& a, const GTLRectInt& b) { return gtl::area(a) > gtl::area(b); });

          int llx = gtl::xl(violation_rect_list[0]);
          int lly = gtl::yl(violation_rect_list[0]);
          int urx = gtl::xh(violation_rect_list[0]);
          int ury = gtl::yh(violation_rect_list[0]);
  ...
  //   DRCLOG.info(Loc::current(),"min area num: ",min_area_drc);
}
