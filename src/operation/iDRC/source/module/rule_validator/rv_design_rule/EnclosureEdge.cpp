...
判断within和parallel字段是否满足规则
  */
bool checkRuleCondiction(PlanarRect a, PlanarRect b, gtl::direction_2d direction, int rule_whthin_value, int rule_parallel_value)
{
  int within_value;
  int parallel_value;
  ...
  PROPERTY LEF58_ENCLOSUREEDGE "
    ...
    思路：
      对于rule1：
        max_rect，获得rect；
        判断rect的width；
        膨胀，沿着width方向膨胀，利用Rtree找到周围的其他rect，判断 prl和within
        width、prl、within均满足？ 找到rect上via，计算via到该边的overhang？
        except字段的判断可能有问题
...
// 工具类函数 Rtree
  ...
  // rule1的EXCEPTTWOEDGES字段判断
  auto isHaveSamePrl = [](std::map<int32_t, bgi::rtree<std::pair<BGRectInt, int32_t>, bgi::quadratic<16>>>& _query_tree, int32_t layer_idx,
                          GTLRectInt current_gtl_rect, gtl::orientation_2d orientation, int rule_within_value) {
    PlanarRect current_planar_rect = DRCUTIL.convertToPlanarRect(current_gtl_rect);

    // 通用处理函数，处理一对方向
    auto checkDirectionPair = [&](gtl::direction_2d dir1, gtl::direction_2d dir2) -> bool {
      int max_prl_value1 = 0;
      int max_prl_value2 = 0;

      // 提取方向信息
      bool is_vertical = dir1.to_int() >= 2;  // NORTH/SOUTH 是垂直方向

      // 处理第一个方向
      GTLRectInt bloat_rect1 = current_gtl_rect;
      gtl::bloat(bloat_rect1, dir1, rule_within_value);
      BGRectInt bloat_bg_rect1 = DRCUTIL.convertToBGRectInt(bloat_rect1);

      std::vector<std::pair<BGRectInt, int32_t>> rect_result1;
      _query_tree[layer_idx].query(bgi::intersects(bloat_bg_rect1), std::back_inserter(rect_result1));

      for (auto& [bg_rect, rect_net_idx] : rect_result1) {
        PlanarRect planar_rect = DRCUTIL.convertToPlanarRect(bg_rect);

        // 跳过重叠的矩形
        if (DRCUTIL.isClosedOverlap(current_planar_rect, planar_rect)) {
          continue;
        }
        ...
        if (is_vertical) {
          within_value
              = (dir1 == gtl::NORTH) ? (planar_rect.get_ll_y() - current_planar_rect.get_ur_y()) : (current_planar_rect.get_ll_y() - planar_rect.get_ur_y());
        } else {
          within_value
              = (dir1 == gtl::EAST) ? (planar_rect.get_ll_x() - current_planar_rect.get_ur_x()) : (current_planar_rect.get_ll_x() - planar_rect.get_ur_x());
        }

        if (within_value > rule_within_value) {
          continue;
        }
        ...
        int parallel_value;
        if (is_vertical) {
          parallel_value = std::min(current_planar_rect.get_ur_x(), planar_rect.get_ur_x()) - std::max(current_planar_rect.get_ll_x(), planar_rect.get_ll_x());
        } else {
          parallel_value = std::min(current_planar_rect.get_ur_y(), planar_rect.get_ur_y()) - std::max(current_planar_rect.get_ll_y(), planar_rect.get_ll_y());
        }

        max_prl_value1 = std::max(parallel_value, max_prl_value1);
      }

      // 处理第二个方向 (与上面相同的逻辑)
      GTLRectInt bloat_rect2 = current_gtl_rect;
      gtl::bloat(bloat_rect2, dir2, rule_within_value);
      BGRectInt bloat_bg_rect2 = DRCUTIL.convertToBGRectInt(bloat_rect2);

      std::vector<std::pair<BGRectInt, int32_t>> rect_result2;
      _query_tree[layer_idx].query(bgi::intersects(bloat_bg_rect2), std::back_inserter(rect_result2));

      for (auto& [bg_rect, rect_net_idx] : rect_result2) {
        PlanarRect planar_rect = DRCUTIL.convertToPlanarRect(bg_rect);
        ...
        if (is_vertical) {
          within_value
              = (dir2 == gtl::NORTH) ? (planar_rect.get_ll_y() - current_planar_rect.get_ur_y()) : (current_planar_rect.get_ll_y() - planar_rect.get_ur_y());
        } else {
          within_value
              = (dir2 == gtl::EAST) ? (planar_rect.get_ll_x() - current_planar_rect.get_ur_x()) : (current_planar_rect.get_ll_x() - planar_rect.get_ur_x());
        }
        ...
        max_prl_value2 = std::max(parallel_value, max_prl_value2);
      }

      // 检查两个方向的最大平行值是否相等且不为0
      return (max_prl_value1 != 0 && max_prl_value2 != 0 && max_prl_value1 == max_prl_value2);
    };

    // 根据orientation调用相应的方向对
    return (orientation == gtl::HORIZONTAL) ? checkDirectionPair(gtl::WEST, gtl::EAST) : checkDirectionPair(gtl::NORTH, gtl::SOUTH);
  };
...
/// rule1
  ...
  for (auto& [routing_layer_idx, net_gtl_all_maxrect_list] : routing_layer_net_gtl_all_maxrect_list) {
    ...
    for (auto& [net_idx, rect_list] : net_gtl_all_maxrect_list) {
      for (GTLRectInt& current_gtl_rect : rect_list) {
        PlanarRect current_planar_rect = DRCUTIL.convertToPlanarRect(current_gtl_rect);
        auto current_rect_width_orientation = gtl::guess_orientation(current_gtl_rect).get_perpendicular();
        int current_rect_width = gtl::delta(current_gtl_rect, current_rect_width_orientation);

        // 判断规则中的EXCEPTTWOEDGES
        if (current_rect_width == enclosure_edge_rules1[0]._minwidth
            && isHaveSamePrl(routing_layer_all_query_tree, routing_layer_idx, current_gtl_rect, current_rect_width_orientation,
                             enclosure_edge_rules1[0]._within)) {
          continue;
        }
        ...
        for (int32_t cut_layer_idx : cut_layer_idx_list) {
          // 判断rule1的ABOVE字段
          if (rule_index != enclosure_edge_rules1.size() - 1 && routing_layer_idx == 1 && cut_layer_idx == 0) {
            continue;
          }
          // 查找矩形内的通孔
          std::vector<std::pair<BGRectInt, int32_t>> vias_in_current_rect
              = queryRectbyRtreeWithWithin(cut_layer_all_query_tree, cut_layer_idx, gtl::xl(current_gtl_rect), gtl::yl(current_gtl_rect),
                                           gtl::xh(current_gtl_rect), gtl::yh(current_gtl_rect));
          ...
          // 沿着width边检查
          auto checkDirectionOverhang = [&](const std::vector<gtl::direction_2d>& directions) {
            for (auto& bloating_direction : directions) {
              // 膨胀当前矩形
              GTLRectInt bloat_current_rect = current_gtl_rect;
              gtl::bloat(bloat_current_rect, bloating_direction, applicable_rule._within);
              ...
              for (auto& [around_bg_rect, around_rect_net_idx] : around_rect_result) {
                ...
                // 检查rule1的within和paraller字段
                if (checkRuleCondiction(current_planar_rect, around_planar_rect, bloating_direction, applicable_rule._within, applicable_rule._parlength)) {
                  // 检查overhang字段
                  auto isViaViolatingOverhang = [&](const PlanarRect& via_rect, gtl::direction_2d direction) -> bool {
                    if (direction == gtl::WEST) {
                      return abs(via_rect.get_ll_x() - current_planar_rect.get_ll_x()) < applicable_rule._overhang;
                    } else if (direction == gtl::EAST) {
                      return abs(current_planar_rect.get_ur_x() - via_rect.get_ur_x()) < applicable_rule._overhang;
                    } else if (direction == gtl::NORTH) {
                      return abs(current_planar_rect.get_ur_y() - via_rect.get_ur_y()) < applicable_rule._overhang;
                    } else if (direction == gtl::SOUTH) {
                      return abs(via_rect.get_ll_y() - current_planar_rect.get_ll_y()) < applicable_rule._overhang;
                    }
                    return false;
                  };
                  ...
                  for (auto& via_in_current_rect : vias_in_current_rect) {
                    ...
                    if (isViaViolatingOverhang(via_planar_Rect, bloating_direction)) {
                      // 创建违规
                      Violation violation;
                      violation.set_violation_type(ViolationType::kEnclosureEdge);
                      violation.set_is_routing(true);
                      violation.set_violation_net_set({net_idx, around_rect_net_idx});
          ...
          if (current_rect_width_orientation == gtl::HORIZONTAL) {
            checkDirectionOverhang({gtl::WEST, gtl::EAST});
          } else {
            checkDirectionOverhang({gtl::NORTH, gtl::SOUTH});
          }
        }
      }
    }
          ...
          if (edge_length_list[i] <= enclosure_edge_rule2._convexlength && convex_corner_list[i] == true && convex_corner_list[pre_i] == true) {
            if ((edge_length_list[pre_i] <= enclosure_edge_rule2._adjacentlength && convex_corner_list[pre_pre_i] == true
              ...
              for (int32_t cut_layer_idx : cut_layer_idx_list) {
                ...
                // 检查特定方向的相邻shape
                auto checkDirectionCondition = [&](gtl::direction_2d direction) -> std::pair<bool, int32_t> {
                  ...
                  for (auto& [around_bg_rect, around_rect_net_idx] : around_results) {
                    ...
                    // 检查规则条件
                    if (checkRuleCondiction(query_planar_rect, around_planar_rect, direction, enclosure_edge_rule2._parWithin, 0)) {
                ...
                // 检查两个方向
                auto [check_condition1, _] = checkDirectionCondition(bloating_direction_1);
                if (check_condition1) {
                  auto [check_condition2, adjacent_net_idx] = checkDirectionCondition(bloating_direction_2);
