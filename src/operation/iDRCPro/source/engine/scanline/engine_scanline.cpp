...
void DrcEngineScanline::fillResultToBasicPoint(ScanlineStatus& status, DrcBasicPoint* basepoint_forward, DrcBasicPoint* basepoint_backward,
...
bool DrcEngineScanline::tryCreateNonEndpoint(ScanlineStatus& status, ScanlinePoint* point)
{
  if (!status.isPointInCurrentBucket(point)) {
    ...
    _data_manager->addBasicPoint(new_point);
