...
void DrcRuleBuilder::initRoutingLayerRules()
{
  ...
  for (auto* idb_layer : idb_layers) {
    ...
    rule_routing_layers[idb_routing_layer->get_id()] = rule_layer;
...
void DrcRuleBuilder::buildRoutingLayerSpacing(ConditionRuleLayer* rule_layer, idb::IdbLayerRouting* idb_routing_layer)
{
  ...
  auto build_spacing_jogtojog = [](idb::IdbLayerRouting* idb_routing_layer, RulesMapSpacing* rule_map) {
    ...
    ConditionRuleJogToJog* rule_jog
        = new ConditionRuleJogToJog(RuleType::kSpacingJogToJog, idb_rule_jog->get_jog_to_jog_spacing(), idb_rule_jog.get());
