...
bool initConfigMapByJSON(const std::string& config, std::map<std::string, std::any>& config_map)
{
  ...
  for (size_t i = 0; i < _config_list.size(); i++) {
    tcl::TclUtil::getValue(json, config_map, _config_list[i].first, _config_list[i].second);
  }
  tcl::TclUtil::closeFileStream(config_file);
  return true;
}
