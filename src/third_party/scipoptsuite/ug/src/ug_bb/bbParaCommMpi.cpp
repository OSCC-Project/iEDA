...
MPI_Datatype
BbParaCommMpi::datatypes[TYPE_LIST_SIZE];

const char *
BbParaCommMpi::tagStringTable[] = {
...
bool
BbParaCommMpi::tagStringTableIsSetUpCoorectly(
      )
{
   return ( sizeof(tagStringTable)/sizeof(char*) == (N_BB_MPI_TAGS - N_MPI_TAGS) );
}

const char *
BbParaCommMpi::getTagString(
      int tag                 /// tag to be converted to string
      )
{
   assert( tag >= 0 && tag < N_BB_MPI_TAGS );
...
BbParaCommMpi::createParaCalculationState(
      )
{
...
ParaTask *
BbParaCommMpi::createParaTask(
      )
{
   return new BbParaNodeMpi();
}

ParaTask *
BbParaCommMpi::createParaNode(
               TaskId inNodeId,
...
ParaSolverState *
BbParaCommMpi::createParaSolverState(
      )
{
   return new BbParaSolverStateMpi();
}
...
BbParaCommMpi::createParaSolverTerminationState(
      )
{
   return new BbParaSolverTerminationStateMpi();
}
