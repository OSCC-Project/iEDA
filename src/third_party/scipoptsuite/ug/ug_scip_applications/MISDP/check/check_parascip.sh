...
do
    ...
    then
        ...
        then
            ...
            if [ "$SDPSETTINGS" != "" ]
            then
                echo "ulimit -t $HARDTIMELIMIT s;mpiexec -np $PROC ../$BINNAME $TMPFILE $i -sl $SDPSETTINGS -sr $SDPSETTINGS -s $SDPSETTINGS"
                bash -c "ulimit -t $HARDTIMELIMIT s;mpiexec -np $PROC ../$BINNAME $TMPFILE $i -sl $SDPSETTINGS -sr $SDPSETTINGS -s $SDPSETTINGS" 2>>$ERRFILE
            else
                echo "ulimit -t $HARDTIMELIMIT s;mpiexec -np $PROC ../$BINNAME $TMPFILE $i"
