...
void VecFeatureDrc::feature_graph()
{
  markWires();
  markNets();
}

void VecFeatureDrc::feature_patch()
{
}

void VecFeatureDrc::markNodes()
{
  ieda::Stats stats;

  LOG_INFO << "Vectorization mark nodes drc start...";
...
#pragma omp parallel for schedule(dynamic)
  ...
  LOG_INFO << "Vectorization mark nodes drc end...";

  _drc_id_to_type = std::move(drc_id_to_type);
}

void VecFeatureDrc::markWires()
{
  ieda::Stats stats;
  ...
  LOG_INFO << "Vectorization mark wire drc start...";
...
#pragma omp parallel for schedule(dynamic)
  ...
  LOG_INFO << "Vectorization memory usage " << stats.memoryDelta() << " MB";
  LOG_INFO << "Vectorization elapsed time " << stats.elapsedRunTime() << " s";
  LOG_INFO << "Vectorization mark wire drc end...";
}

void VecFeatureDrc::markNets()
{
  mark_drc_num = 0;
  ...
  LOG_INFO << "Vectorization mark net drc start...";
