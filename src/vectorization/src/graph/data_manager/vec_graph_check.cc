...
void GraphCheckerBase::writeToPy(VecNet& net, const std::string& path) const
{
...
void GraphCheckerBase::writeToPy(const Graph& graph, VecNet& net, const std::string& path, const bool& mark_break,
                                 const bool& mark_pin_id) const
{
  ...
  std::unordered_map<WireKey, VecNetWire, WireKeyHash> wire_map;
  for (auto& wire : wires) {
...
bool VecNetChecker::isLocalConnectivity(VecNet& net) const
{
...
bool VecNetChecker::isLocalConnectivity(VecNetWire& wire) const
{
  std::vector<std::pair<VecNode*, VecNode*>> paths = wire.get_paths();
  auto is_same_xy = [](VecNode* node1, VecNode* node2) { return node1->get_x() == node2->get_x() && node1->get_y() == node2->get_y(); };
  if (paths.empty()) {
    return false;
  }
  if (paths.size() == 1) {
...
Graph VecNetChecker::convertToGraph(VecNet& net) const
{
  ...
  Graph graph(next_id);
  std::unordered_map<Edge, VecNetWire, EdgeHash> edge_to_wire;
  ...
  std::vector<VecNetWire> updated_wires;
  for (auto e : boost::make_iterator_range(boost::edges(graph))) {
...
void VecNetChecker::writeToDot(const Graph& graph, const std::string& path) const
{
  GraphCheckerBase::writeToDot(graph, path);
}
void VecNetChecker::writeToPy(const Graph& graph, VecNet& net, const std::string& path, const bool& mark_break,
                              const bool& mark_pin_id) const
{
  GraphCheckerBase::writeToPy(graph, net, path, mark_break, mark_pin_id);
}
bool VecLayoutChecker::checkLayout(std::map<int, VecNet> net_map)
{
  int success_num = 0;
  int total = 0;
  auto checker = VecNetChecker();
...
void VecLayoutChecker::checkPinConnection(std::map<int, VecNet> net_map)
{
  ieda::Stats stats;

  LOG_INFO << "Vectorization check pin connections for routing layer start...";
  ...
  LOG_INFO << "Vectorization check pin connections for routing layer end...";
}
