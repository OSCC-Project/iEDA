...
bool VecGraphDataManager::buildGraphData(bool is_placement_mode)
{
  ...
  for (size_t net_id = 0; net_id < idb_nets.size(); ++net_id) {
    ...
    auto* vec_net = layout_graph.get_net(net_id);
    vec_net->clearWire();
    vec_net->set_routing_graph(routing_graph);
#if debug_error
    ...
    for (auto edge : boost::make_iterator_range(boost::edges(wire_graph))) {
      ...
      /// add wire
      {
        ...
        vec_wire.set_start(node1);
        vec_wire.set_end(node2);
#if debug_error
...
#if debug_error
      VecNode* bk_start = nullptr;
      VecNode* bk_end = nullptr;
      int i = 0;
#endif
...
#if debug_error
    if (pin_ids_wires.size() != vec_net->get_pin_ids().size()) {
      LOG_WARNING << "Warning, pin size mismatch, name : " << _layout->findNetName(net_id) << " ,net id : " << net_id
                  << " ,net num : " << vec_net->get_pin_ids().size() << " ,wire num : " << pin_ids_wires.size()
