...
void VecShapeTableView::resizeEvent(QResizeEvent* event)
{
  QTableView::resizeEvent(event);
  _setColumnWidths();
}
void VecShapeTableView::_setColumnWidths()
{
...
void VecColorItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
...
bool VecColorItemDelegate::editorEvent(QEvent* event, QAbstractItemModel* model, const QStyleOptionViewItem& option,
...
VecGraphWidget::VecGraphWidget(QWidget* parent)
      ...
      _previous_filter_text("")
{
  ...
  _table_view = new VecShapeTableView(_filtering_group_box);
  ...
  auto* color_delegate = new VecColorItemDelegate(_table_view);
  ...
  connect(_hide_all_check_box, &QCheckBox::stateChanged, this, &VecGraphWidget::onHideAllChecked);
  connect(_show_all_check_box, &QCheckBox::stateChanged, this, &VecGraphWidget::onShowAllChecked);
  connect(_unified_color_button, &QToolButton::clicked, this, &VecGraphWidget::onUnifiedColorClicked);

  _search_timer = new QTimer(this);
  _search_timer->setSingleShot(true);
  _search_timer->setInterval(300);
  connect(_search_timer, &QTimer::timeout, this, &VecGraphWidget::onSearchTimeout);
  connect(_search_line_edit, &QLineEdit::textChanged, this, [this](const QString&) { _search_timer->start(); });

  connect(_rotation_button, &QToolButton::clicked, this, &VecGraphWidget::toggleRotationMode);
  connect(_reset_view_button, &QToolButton::clicked, this, &VecGraphWidget::resetView);
  connect(_class_model, &QStandardItemModel::dataChanged, this, &VecGraphWidget::onClassModelDataChanged);
...
void VecGraphWidget::addWire(float x1, float y1, float z, float x2, float y2, float, const std::string& comment,
...
void VecGraphWidget::addRect(float x1, float y1, float z, float x2, float y2, float, const std::string& comment,
                             const std::string& shape_class, const QVector3D& color)
{
  Shape shape;
  shape.type = Rect;
...
void VecGraphWidget::addVia(float x, float y, float z1, float z2, const std::string& comment, const std::string& shape_class,
...
void VecGraphWidget::showAxes()
{
  _axes_visible = true;
  update();
}

void VecGraphWidget::initView()
{
  if (_shapes_by_class.isEmpty()) {
    _zoom_factor = 1.0f;
    _pan_x = 0.0f;
...
void VecGraphWidget::initializeGL()
{
...
void VecGraphWidget::resizeGL(int w, int h)
{
  glViewport(0, 0, w, h);
...
void VecGraphWidget::mousePressEvent(QMouseEvent* event)
{
...
void VecGraphWidget::mouseMoveEvent(QMouseEvent* event)
{
  if (event->pos().x() < _control_panel->width()) {
    return;
  }
...
void VecGraphWidget::wheelEvent(QWheelEvent* event)
{
...
bool VecGraphWidget::event(QEvent* e)
{
...
void VecGraphWidget::resizeEvent(QResizeEvent* event)
{
...
void VecGraphWidget::toggleRotationMode(bool checked)
{
  _rotation_mode = checked;
  _rotation_button->setText(checked ? "Rotation Mode (On)" : "Rotation Mode (Off)");
}

void VecGraphWidget::resetView()
{
  initView();
}

void VecGraphWidget::onHideAllChecked(int state)
{
  if (state == Qt::Checked) {
...
void VecGraphWidget::onShowAllChecked(int state)
{
  if (state == Qt::Checked) {
...
void VecGraphWidget::onUnifiedColorClicked()
{
...
void VecGraphWidget::onSearchTimeout()
{
...
void VecGraphWidget::onClassModelDataChanged(const QModelIndex& top_left, const QModelIndex& bottom_right, const QVector<int>& roles)
{
...
void VecGraphWidget::_ensureClassInModel(const QString& class_name, const QVector3D& default_color)
{
...
QVector3D VecGraphWidget::_project(const QVector3D& obj, const QMatrix4x4& model_view, const QMatrix4x4& proj)
{
