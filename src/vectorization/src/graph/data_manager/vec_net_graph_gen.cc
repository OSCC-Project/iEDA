...
WireGraph VecNetGraphGenerator::buildGraph(idb::IdbNet* idb_net) const
{
...
std::vector<WireGraph> VecNetGraphGenerator::buildGraphs() const
{
...
bool VecNetGraphGenerator::isCornerCase(idb::IdbNet* idb_net) const
{
...
WireGraph VecNetGraphGenerator::buildCornerCaseGraph(idb::IdbNet* idb_net) const
{
...
TopoGraph VecNetGraphGenerator::buildTopoGraph(idb::IdbNet* idb_net) const
{
...
TopoGraph VecNetGraphGenerator::buildTopoGraph(idb::IdbSpecialNet* idb_net) const
{
  // LOG_INFO << "Net name : " << idb_net->get_net_name();
...
LayoutShapeManager VecNetGraphGenerator::buildShapeManager(const TopoGraph& graph) const
{
...
void VecNetGraphGenerator::buildConnections(TopoGraph& graph) const
{
...
bool VecNetGraphGenerator::checkConnectivity(const TopoGraph& graph) const
{
...
WireGraph VecNetGraphGenerator::buildWireGraph(const TopoGraph& graph) const
{
  WireGraph wire_graph;
...
std::vector<WireGraphVertex> VecNetGraphGenerator::canonicalizeCycle(const std::vector<WireGraphVertex>& cycle) const
{
  auto forward = cycle;
  auto backward = cycle;
...
void VecNetGraphGenerator::dfsFindCycles(const WireGraph& graph, WireGraphVertex start, WireGraphVertex current, std::vector<bool>& visited,
...
std::vector<std::vector<WireGraphVertex>> VecNetGraphGenerator::findAllCycles(const WireGraph& graph) const
{
...
void VecNetGraphGenerator::breakCycle(WireGraph& graph) const
{
  if (!hasCycle(graph)) {
    return;
  }
...
void VecNetGraphGenerator::innerConnectivityCompletion(const TopoGraph& graph, WireGraph& wire_graph,
...
void VecNetGraphGenerator::buildVirtualWire(const TopoGraph& graph, WireGraph& wire_graph, WireGraphVertexMap& point_to_vertex) const
{
...
void VecNetGraphGenerator::markPinVertex(const TopoGraph& graph, WireGraph& wire_graph) const
{
  auto shape_manager = LayoutShapeManager();
...
void VecNetGraphGenerator::reduceWireGraph(WireGraph& graph, const bool& retain_pin) const
{
  // Step 1: Prepare
...
bool VecNetGraphGenerator::hasCycleUtil(const WireGraph& graph, WireGraphVertex v, std::vector<bool>& visited, WireGraphVertex parent) const
{
  visited[v] = true;
...
bool VecNetGraphGenerator::hasCycle(const WireGraph& graph) const
{
  size_t num_vertices = boost::num_vertices(graph);
  std::vector<bool> visited(num_vertices, false);
...
bool VecNetGraphGenerator::checkConnectivity(const WireGraph& graph) const
{
  std::vector<int> component(boost::num_vertices(graph));
  auto num = boost::connected_components(graph, component.data());
  if (num > 1) {
...
bool VecNetGraphGenerator::checkDriverIsTreeRoot(const TopoGraph& topo_graph, const WireGraph& wire_graph) const
{
...
std::vector<std::vector<LayoutDefPoint>> VecNetGraphGenerator::generateShortestPath(const std::vector<LayoutDefPoint>& points,
...
std::vector<std::vector<LayoutDefPoint>> VecNetGraphGenerator::findByDijkstra(const std::vector<LayoutDefPoint>& points,
...
std::vector<LayoutDefPoint> VecNetGraphGenerator::generateCrossroadsPoints(const LayoutDefPoint& p, const LayoutDefRect& rect) const
{
...
LayoutDefPoint VecNetGraphGenerator::generatePointPivot(const LayoutDefPoint& p, const LayoutDefRect& rect) const
{
  auto x = getX(p);
  auto y = getY(p);
  auto z = getZ(p);
...
LayoutDefPoint VecNetGraphGenerator::generateSegPivot(const LayoutDefSeg& seg, const LayoutDefRect& rect) const
{
  LayoutDefPoint center = getCenter(seg);
  return generatePointPivot(center, rect);
}
void VecNetGraphGenerator::toPy(const TopoGraph& graph, const std::string& path) const
{
...
void VecNetGraphGenerator::toPy(const WireGraph& graph, const std::string& path) const
{
  std::ofstream file(path);
  file << "import plotly.graph_objects as go\n";
  file << "import matplotlib.cm as cm\n";
